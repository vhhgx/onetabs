...
export async function retrieveChunkedData(key) {
  try {
    ...
    if (typeof chrome !== 'undefined' && chrome.storage && chrome.storage.local) {
      const result = await new Promise((resolve) => chrome.storage.local.get(`${key}_metadata`, resolve))
    ...
    for (let i = 0; i < metadata.chunks; i++) {
      ...
      if (typeof chrome !== 'undefined' && chrome.storage && chrome.storage.local) {
        const result = await new Promise((resolve) => chrome.storage.local.get(`${key}_chunk_${i}`, resolve))
